...
class RectangleProcessor:
    ...
    def process(self, image: Image.Image, debug_name: str = "detected_rectangle.png") -> ProcessResult:
        ...
        min_area_val = img_area * self.min_quad_area_ratio
        max_area_val = img_area * 0.2
        ...
        for cnt in contours:
            ...
            if len(approx) == 4 and cv2.isContourConvex(approx):
                ...
                if min_area_val < area < max_area_val:
                    ...
                    candidates.append((approx, area, ratio))

        if not candidates:
            return self._empty_result(original, debug_name, "Square not found")

        candidates.sort(key=lambda x: x[2], reverse=True)
        best_corners = candidates[0][0].reshape(4, 2).astype(np.float32)

        corners = self._order_quad_points(best_corners / scale)
...
if __name__ == "__main__":
    RectangleProcessor().process(Image.open("scan_screen.png"))